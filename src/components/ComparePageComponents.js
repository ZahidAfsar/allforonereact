...
import { Compare } from '../Services/dataServices.js'

export default function ComparePageComponents() {

    const [numOneString, setNumOneString] = useState('');
    const [numTwoString, setNumTwoString] = useState('');
    const [result, setResult] = useState('');
  
    const getResult = async () => {
      const response = await Compare(numOneString, numTwoString);
      setResult(response)
    }
  ...
  <h1 className="text-center font-Modak text-7xl text-custom-orange drop-shadow-[0_1.2px_1.2px_rgba(255,255,255,4)] pt-16">
    Mini Challenge 4
  </h1>
  <h1
    id="SayHelloText"
  ...
  >
    {result === "" ?     "... is greater or less than ..." : result}
  </h1>
  <div className="container-fluid hidden sm:block">
    <div className="flex justify-center">
      <div className="mt-16">
        <input onChange={(e) => setNumOneString(e.target.value)} 
          type="text"
      ...
      <div className="mt-16 ml-2">
        <input onChange={(e) => setNumTwoString(e.target.value)}
          type="text"
